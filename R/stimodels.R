...
#' # during 100 consecutive days in 22 emergence traps along a stream. The 22 traps (sites)
#' # form a regular transect, with geographic positions 1 to 22. The original daily data 
#' # collected at each site were pooled into 10 survey periods for the study of Legendre et 
#' # al. (2010) in order to reduce the very high proportion of zeros in the original data 
#' # matrix. Order of the observations in the data set: the 22 traps (sites) are nested 
#' # within the survey weeks, as required by the 'stimodels' and 'quicksti' 
#' # functions.
#' 
#' data(trichoptera)
#' 
...
#' # variation can be done for individual times using simple RDA against dbMEM.S variables. 
#' # Likewise, tests of temporal variation can be done for individual sites using simple RDA 
#' # against dbMEM.T variables. A global test of the hypothesis that none of the times shows 
#' # a significant spatial structure can be done with model 6a. For a global test of 
#' # temporal structure at the various sites, use model 6b.
#' 
...
#' # tests of existence of a spatial (at least in one of the time periods) and temporal (at 
#' # least at one of the sites) structures using models 6a and 6b, respectively.
#' 
...
#'
#' # Results should be identical to those in output out.1 of Example 1, except for 
                        ...
                        Tfixed=TRUE, COD.S=NULL, COD.T=NULL, save.X=FALSE, print.res=TRUE)
{
  ...
  if(model!="2" && model!="3a" && model!="3b" && model!="4" && model!="5" 
    ...
    stop(paste("Unrecognized model ",model,
               "; 'model' should be '2', '3a', '3b', '4', '5', '6a','6b' or '7'.", sep=""),
         call.=FALSE)
  }
  ...
  aa <- system.time( {
    ...
    if(!skip.Manova) {   # Begin of "(!skip.Manova)"
      ...
      res <- manovRDa(Y=Y,s=s,tt=tt,S.mat=XS,T.mat=XT,STI.mat=XSTI, Sfixed=Sfixed, 
                      Tfixed=Tfixed, S.test=test.S, T.test=test.T, STI.test=test.STI, model = model, 
...
'manovRDa' <- function(Y, s, tt, S.mat=NULL, T.mat=NULL, STI.mat=NULL, Sfixed=TRUE, 
                       Tfixed=TRUE, S.test=TRUE, T.test=TRUE, STI.test=TRUE, model = "5", nperm=999, save.X)
